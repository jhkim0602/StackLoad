  ...
  useEffect(() => {
    async function loadStats() {
      try {
        ...
        // 카테고리별 통계 계산
        const categoryCount = techs.reduce(
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="text-center p-4 bg-gray-100 rounded-xl animate-pulse"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat) => {
          ...
          return (
            <div
              key={stat.category}
              className={`text-center p-4 rounded-xl border ${stat.color} hover:shadow-md transition-all duration-200`}
            >
